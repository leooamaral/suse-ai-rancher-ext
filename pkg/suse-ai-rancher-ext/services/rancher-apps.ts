  ...
  try {
    ...
    try {
      ...
      if (helmSecrets.length > 0) {
        ...
        const detailUrl = `const detailUrl = `/k8s/clusters/${encodeURIComponent(clusterId)}/v1/secrets/${encodeURIComponent(namespace)}/${encodeURIComponent(secretName)}?exclude=metadata.managedFields&includeHelmData=true`;`;
